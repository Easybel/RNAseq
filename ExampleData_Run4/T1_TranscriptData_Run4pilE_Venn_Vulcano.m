...
if plotVulcano == "ON"
    ...
    for i=1:numel(expName)
        ...
        set(gca,'FontSize', 16,'YScale','lin');
        hold on
        title("Condition:" + " " + reference + " vs " + conditions(i))
        ...
        s(i) = scatter([collectExps(i).expData(:).log2FoldChange],pV_log10,20,'Marker','o',...
        ...
        u(i) = scatter([collectExps(i).expData(Venndata(i).mask_up).log2FoldChange],pV_log10(Venndata(i).mask_up),20,'Marker','o',...
            'MarkerFaceColor',red,'MarkerEdgeColor','none','DisplayName','up');
        % blue for down regulated
        d(i) = scatter([collectExps(i).expData(Venndata(i).mask_dw).log2FoldChange],pV_log10(Venndata(i).mask_dw),20,'Marker','o',...
        ...
        text((XLim + 0.5)/2,YLim*0.9 ,string(upFrac(i)) + " " + "%",'Color',red,'FontSize',14)
        text((-XLim - 0.5)/2 - 0.4,YLim*0.9 ,string(downFrac(i)) + " " + "%",'Color',blue,'FontSize',14)
...
legend(compareVenn)
title('upregulated genes in DG4 biofilm vs ...')
...
legend(compareVenn)
title('downregulated genes in DG4 biofilm vs ...')

%% save the data for the other script
outPut = "/home/isabel/Documents/Doktorarbeit_Mai2022/P5_Ngo_fromSciebo/RNA/2022March_Run4/4_pilE_Expression/3_Result_Transk_wminO20Data/matlabOutputs/";

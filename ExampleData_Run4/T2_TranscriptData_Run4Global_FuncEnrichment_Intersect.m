...
basePath = "/home/isabel/Documents/Doktorarbeit_Mai2022/P5_Ngo_fromSciebo/RNA/2022March_Run4/1b_Analysis_naiv_woMS11_DpilT/2b_Results_DGE/";

conditions = ["pilEKO" "NG17" "NG24" "NG32"];%"Cef15min", ,"Azi15min", "DMSO60min"];
reference = "DG4 biofilm";

inPut = "/home/isabel/Documents/Doktorarbeit_Mai2022/P5_Ngo_fromSciebo/RNA/2022March_Run4/1b_Analysis_naiv_woMS11_DpilT/2b_Results_DGE/matlabOutputs/";
collectPath = inPut + "20220906_collectExps.mat";
VennPath = inPut + "20220906_Venndata.mat";
...
% where to save to??
outPlots = basePath + "/RNA/2021Feb_Run2/2b_Results_DGE/plots/";
...
intersect_cond = ["pilEKO" "NG17" "NG32"];
...
Venndata = Venndata(find(contains([Venndata.condition],intersect_cond)));
...
for i=1:2
    inter1 = Venndata(1).("ltag_" + regulation(i))(contains(Venndata(1).("ltag_" + regulation(i)),Venndata(2).("ltag_" + regulation(i))));
    intersect.(regulation(i) + "_lt_Ngo") = Venndata(3).("ltag_" + regulation(i))(contains(Venndata(3).("ltag_" + regulation(i)),inter1));
    clear inter1 
end

intersect.(regulation(3) + "_lt_Ngo") = [intersect.(regulation(1) + "_lt_Ngo") intersect.(regulation(2) + "_lt_Ngo")];

% we have the lt in Ngo, what is the index in FA1090, for which we have the
% categories??
for i=1:numel(regulation)
intersect_mask        = contains([collectExps_wCat(1).expData.locustag_in_Ngo],intersect.(regulation(i) + "_lt_Ngo")');
intersect.(regulation(i) + "_lt_FA1090") = collectExps_wCat(1).expData(intersect_mask);
clear intersect_mask
end
...
for i=1:numel(regulation)
    GeneIntersect = [intersect.(regulation(i) + "_lt_FA1090")];
